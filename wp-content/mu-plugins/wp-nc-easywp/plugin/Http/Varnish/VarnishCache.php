    ...
    public function purge($url)
    {
        try {
            ...
            if (!isset($parsedUrl['host'])) {
                throw new Exception(__('Invalid host in purge URL', \WPNCEasyWP::TEXTDOMAIN));
            }
            ...
            wp_remote_request($urlToPurge, ['method' => 'PURGE', 'headers' => $headers]);
    ...
    public function doActionMenu($silent = false)
    {
        ...
        if (!$silent) {
            add_action('admin_notices', function () {
                ...
                echo '<p>';
                _e('Varnish cache emptied!', \WPNCEasyWP::TEXTDOMAIN);
                echo '</p>';
                echo '</div>';
            });
        }

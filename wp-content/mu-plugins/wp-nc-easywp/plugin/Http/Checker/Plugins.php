...
class Plugins
{
    ...
    protected function checkNotAllowed()
    {
        ...
        if (!empty($this->getWillDisabled())) {
            $adminNotice = function () {
                add_action('admin_notices', function () {
                    ...
                    printf('<h4>%s</h4>', _n('The following plugin can\'t be activated:', 'The following plugins can\'t be activated:', count($this->getWillDisabled()), \WPNCEasyWP::TEXTDOMAIN));
                    echo '<ul>';
        ...
        if (!empty($this->getDisabled())) {
            ...
            add_action('admin_notices', function () {
                echo '<div id="message" class="notice notice-success fade is-dismissible">';
                printf('<h4>%s</h4>', _n('The following plugin was disabled', 'The following plugins were disabled', count($this->getDisabled()), \WPNCEasyWP::TEXTDOMAIN));
                echo '<ul>';
        ...
        add_filter(
            ...
            function ($install_actions, $api, $plugin_file) {
                if (in_array($plugin_file, $this->getNotAllowedKeys())) {
                    $install_actions['activate_plugin'] = __('The plugin can\'t be installed', \WPNCEasyWP::TEXTDOMAIN) . '<details>' . $this->getReasonByKey($plugin_file) . '</details>';
                }
        ...
        add_filter(
            ...
            function ($source) {
                ...
                if (in_array($pluginSlug, $this->getNotAllowedSlug())) {
                    $source = new WP_Error(999, __('The plugin can\'t be installed', \WPNCEasyWP::TEXTDOMAIN) . '<details>' . $this->getReasonBySlug($pluginSlug) . '</details>');
                }
    ...
    protected function warningBySlug($slug)
    {
        ...
        $warning = __('The plugin can\'t be installed', \WPNCEasyWP::TEXTDOMAIN);
